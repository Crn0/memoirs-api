...
import { isNotEmpty, validFileType } from '../../../helpers/validators/validators.mjs'
...
const posts_new = [
    ...
    .withMessage('The file extension is not supported. Please upload a file with one of the following extensions: .jpg, .jpeg, .png, .webp')
    .custom((_, { req }) => {
    ...
    asyncHandler(async (req, res, _) => {
        ...
        if(req.file) {
            const image = await Cloudinary.upload(req.file.path, req.user.username, post.id)
